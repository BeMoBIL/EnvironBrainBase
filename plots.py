...
def plot_research_topic(df: pd.DataFrame) -> None:
    ...
    ax2.set_title(
        "Environmental Focus Distribution", pad=8, loc="left", color="#1F1F1F"
    )
...
def plot_paradigm_by_research_object(df: pd.DataFrame) -> None:
    ...
    if not rows:
        print(
            "  No research-object paradigm data found; skipping paradigm_by_research_object plot."
        )
        return
    ...
    summary = summary.reindex(
        [topic for topic in topic_labels if topic in summary.index]
    )
    ...
    ax.set_title(
        "Paradigm Choice by Research Object", loc="left", pad=8, color="#1F1F1F"
    )
    ax.set_xlabel("Number of studies", color="#3A3A3A", labelpad=6)
    ax.set_ylabel("")
...
def plot_study_design_by_research_object(df: pd.DataFrame) -> None:
    ...
    if not rows:
        print(
            "  No research-object study-design data found; skipping study_design_by_research_object plot."
        )
        return
...
def plot_non_replicability_by_topic(df: pd.DataFrame) -> None:
    ...
    if not topic_rows:
        print(
            "  No research topic data found; skipping non_replicability_by_topic plot."
        )
        return
    ...
    summary["non_replicability_pct"] = (
        summary["non_replicable"] / summary["total"] * 100
    )
    ...
    bars = ax.barh(
        summary.index, summary["non_replicability_pct"], color="#0B3C5D", height=0.62
    )
    ...
    ax.set_title(
        "Non-replicability by Research Topic", loc="left", pad=8, color="#1F1F1F"
    )
    ...
    max_pct = (
        float(summary["non_replicability_pct"].max()) if not summary.empty else 0.0
    )
...
def plot_analytic_domain(df: pd.DataFrame) -> None:
    ...
    bars = ax.barh(
        ...
        color=[palette[k] for k in domain_counts.index],
        height=0.62,
    )
    ax.grid(axis="x", color="#D9D7D0", linestyle="-", linewidth=0.6, alpha=0.55)
    ax.set_axisbelow(True)
...
def plot_multimodal_prevalence(df: pd.DataFrame) -> None:
    ...
    if modality_counts.empty:
        print(
            "  No multimodal modality matches found; skipping multimodal_prevalence plot."
        )
        return
    ...
    fig, ax = plt.subplots(figsize=(7.2, 4.8), dpi=300)
    bars = ax.barh(
        modality_counts.index, modality_counts.values, color="#2A9D8F", height=0.62
    )
    ax.grid(axis="x", color="#D9D7D0", linestyle="-", linewidth=0.6, alpha=0.55)
    ax.set_axisbelow(True)
    for spine in ["top", "right", "left", "bottom"]:
    ...
    ax.set_title(
        "Modality Prevalence in Multimodal Studies", loc="left", pad=8, color="#1F1F1F"
    )
...
def plot_multimodal_integration_pie(df: pd.DataFrame) -> None:
    ...
    ax.set_title(
        "EEG-only vs Multimodal Integration", loc="left", pad=8, color="#1F1F1F"
    )
    ax.text(
        0.5,
        -0.08,
...
def plot_consumer_grade_distribution(df: pd.DataFrame) -> None:
    ...
    if grade_col not in df.columns:
        print(
            f"  Column '{grade_col}' not found; skipping consumer_grade_distribution plot."
        )
        return

    grades = (
        df[grade_col].fillna("NA").astype(str).str.strip().str.lower().value_counts()
    )
    counts = pd.Series(
        {
    ...
    if counts.sum() == 0:
        print(
            "  No EEG system grade data found; skipping consumer_grade_distribution plot."
        )
        return
...
def plot_data_availability(df: pd.DataFrame) -> None:
    ...
    availability_frame[year_col] = pd.to_numeric(
        availability_frame[year_col], errors="coerce"
    )
    ...
    availability_frame[availability_col] = pd.to_numeric(
        availability_frame[availability_col], errors="coerce"
    )
    availability_frame["availability_label"] = availability_frame[availability_col].map(
        availability_map
    )
    availability_frame["availability_label"] = availability_frame[
        "availability_label"
    ].fillna("NA")
    ...
    availability_counts = availability_counts.reindex(
        columns=category_order, fill_value=0
    )
    availability_percent = (
        availability_counts.div(availability_counts.sum(axis=1), axis=0) * 100
    )
    ...
    for category in category_order:
        ...
        bars = ax.bar(
            availability_percent.index,
            values,
            bottom=bottom,
            color=availability_palette[category],
            width=0.72,
            label=category,
        )
        for bar, value in zip(bars, values):
            if value >= 10:
                ax.text(
                    ...
                    fontsize=8,
                    color="white"
                    if category in {"Not available", "Available upon request"}
                    else "#1F1F1F",
                )
    ...
    legend_handles = [
        mpatches.Patch(
            color=availability_palette["Not available"], label="Not available"
        ),
        mpatches.Patch(
            color=availability_palette["Available upon request"],
            label="Available upon request",
        ),
        mpatches.Patch(
            color=availability_palette["Openly available"], label="Openly available"
        ),
...
def plot_mean_age(df: pd.DataFrame) -> None:
    ...
    ax.hist(
        mean_age, bins=24, color="#6BA8B8", edgecolor="white", linewidth=0.8, alpha=0.9
    )
    ax.axvspan(
        age_q1,
        age_q3,
        alpha=0.15,
        color="#0B3C5D",
        label=f"IQR: {age_q1:.1f}–{age_q3:.1f}",
    )
    ax.axvline(
        age_median,
        color="#0B3C5D",
    ...
    ax.set_title(
        "Distribution of mean participant age", loc="left", pad=8, color="#1F1F1F"
    )
...
def plot_age_ranges(df: pd.DataFrame) -> None:
    def parse_age_interval(value: object):
        ...
        if "-" in text:
            ...
            if not start_match or not end_match:
                return None
            return {
                "label": f"{start_match.group(0)}-{end_match.group(0)}",
    ...
    interval_records = [
        iv for iv in age_series.apply(parse_age_interval) if iv is not None
    ]
    ...
    interval_table["count"] = (
        interval_table["label"].map(interval_counts).fillna(1).astype(int)
    )
    interval_table = interval_table.assign(
        midpoint=(interval_table["start"] + interval_table["end"]) / 2
    )
    ...
    ax.plot(
        x_range, density_scaled, color="#a3b8c4", linewidth=1.2, alpha=0.75, zorder=1
    )
    ...
    segments = [
        [(row["start"], idx), (row["end"], idx)]
        for idx, row in interval_table.iterrows()
    ]
    lc = LineCollection(
        segments, colors="#0b7285", linewidths=2.2, alpha=0.75, zorder=2
    )
...
def plot_age_coverage(df: pd.DataFrame) -> None:
    ...
    ax.scatter(
        age_axis, coverage.values, s=10, color="#0b7285", alpha=0.65, linewidths=0
    )
...
def plot_publication_year(df: pd.DataFrame) -> None:
    ...
    if years_left.empty or years_right.empty:
        print(
            "  Year distribution does not span the intended break; skipping publication_year plot."
        )
        return
    ...
    if year_median < BREAK_START:
        ax_l.axvline(
            ...
            linestyle="--",
            linewidth=1.5,
            label=f"Median: {int(year_median)}",
            zorder=3,
        )
    else:
        ax_r.axvline(
...
def plot_sex_split(df: pd.DataFrame) -> None:
    ...
    y_top = ax.get_ylim()[1]
    ax.text(
        25, y_top * 0.92, "Female majority", ha="center", fontsize=9, color="#8B5A5A"
    )
    ax.text(75, y_top * 0.92, "Male majority", ha="center", fontsize=9, color="#5A8B8B")

    ax.set_title(
        "Distribution of sex split (% male)", loc="left", pad=8, color="#1F1F1F"
    )
...
def plot_replicability_criteria(df: pd.DataFrame) -> None:
    ...
    if not required_rates:
        print(
            "  No replicability criteria found; skipping replicability_criteria plot."
        )
        return
    ...
    for spine in ["top", "right", "left", "bottom"]:
        ax.spines[spine].set_visible(False)
    ax.set_title(
        "Replicability Criteria Reporting Rates", loc="left", pad=8, color="#1F1F1F"
    )
...
def plot_replicability_summary(df: pd.DataFrame) -> None:
    if "replicable" not in df.columns:
        print(
            "  Replicability scores not available; skipping replicability_summary plot."
        )
        return
    ...
    ax.set_title(
        "Replicable vs Non-replicable Studies", loc="left", pad=8, color="#1F1F1F"
    )
    ax.text(
        0,
        -1.15,
...
def plot_replicability_by_venue(df: pd.DataFrame) -> None:
    if "replicable" not in df.columns:
        print(
            "  Replicability scores not available; skipping replicability_by_venue plot."
        )
        return

    venue_frame = df.copy()
    venue_counts = venue_frame["venue_domain"].value_counts()
    venue_summary = venue_frame.groupby("venue_domain")["replicable"].agg(
        total="size", replicable="sum"
    )
    venue_summary["replicable_pct"] = (
        venue_summary["replicable"] / venue_summary["total"] * 100
    )
    venue_summary = venue_summary.reindex(venue_counts.index).dropna(
        subset=["replicable_pct"]
    )
    ...
    ax.set_title(
        "Replicable Studies by Publishing Venue Domain",
        loc="left",
        pad=8,
        color="#1F1F1F",
    )
    ax.set_xlabel(
        "Replicable share within venue group (%)", color="#3A3A3A", labelpad=6
    )
    ax.set_ylabel("")
    ax.set_xlim(0, 100)
    ax.tick_params(axis="x", colors="#555555", length=0)
...
def plot_top_journals(df: pd.DataFrame, top_n: int = 10) -> None:
    ...
    other_count = (
        int(journal_counts.iloc[top_n:].sum()) if len(journal_counts) > top_n else 0
    )
    if other_count > 0:
    ...
    colors = [
        "#6BA8B8" if label != "Other" else "#A8E6CF" for label in top_counts.index
    ]
    fig, ax = plt.subplots(figsize=(8.0, 5.2), dpi=300)
    ...
    ax.set_title(
        "Top Journals (Remaining as Other)", loc="left", pad=8, color="#1F1F1F"
    )
    ax.set_xlabel("Number of papers", color="#3A3A3A", labelpad=6)
    ax.set_ylabel("")
    ax.tick_params(axis="x", colors="#555555", length=0)
